...
| **🥇 当前最好成绩** | **69.218** —— `sub_288_tta4.csv` = **288 分辨率 + 4 视图 TTA**（见 §16.11） |
| 🥈 上一档 | 66.2456 —— `sub_tta4.csv` = 224 + 4 视图 TTA |
...
| # | 做什么 | 依据 |
| --- | --- | --- |
| **P0** | **`288 + 8 视图`** —— `--tta plain flip wide wide_flip tight tight_flip mid mid_flip`，checkpoint 仍用 `outputs_288/ep20.pt` | 288 是赢的线；视图数在 224 上还没测过，而在 288 上多样性收益更大 |
| **P1** | **320 分辨率**（探针已验证可行，余弦 0.9683） | 288 已经证明「分辨率是结构性变量」；320 是同一方向的下一步。每轮约 7.5 分钟，20 轮约 2.5 小时 |
| **P2** | `sub_tta8.csv`（224 + 8 视图，**已生成待提交**） | 只回答「视图数」这一个问题，且分数天花板低于 69.218 ⇒ **信息量有，但改善不了成绩** |
