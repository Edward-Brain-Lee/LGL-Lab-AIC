...
| **🥇 当前最好成绩** | **70.332** —— `sub_320_tta8.csv` = **320 分辨率 + 8 视图 TTA**（见 §16.13） |
| 🥈 | 69.968 —— `sub_320_tta4.csv` = 320 + 4 视图 TTA（§16.12） |
| 🥉 | 69.218 —— `sub_288_tta4.csv` = 288 + 4 视图 TTA（§16.11） |
| — | 66.2456 —— `sub_tta4.csv` = 224 + 4 视图 TTA（§16.2） |
| 基线 | 64.16 —— `sub_ep20.csv` = 224，无 TTA |
| 初赛得分（参考） | 68.02 —— **不同数据集，不可直接比较**（**现在已经超过了**） |
| 已完成 | `outputs_r2` / `outputs_old`（各 20 ep）、`outputs_ema`（224 + 教师）、`outputs_288`、`outputs_320`、死类诊断、**`infer.py` 并行变换** |
| **下一步** | ① **352 分辨率**（分辨率是至今最强杠杆，且还没见底）② 或继续加视图（递减中，+0.36/4视图） |
...
| 7.8 | **多视图 TTA 的瓶颈在 CPU，不在 GPU** | 8 视图 @288px 跑了 90+ 分钟还没完，最后只能杀掉；GPU 利用率却一直很高 | `infer.py` 的图片变换（`Resize`/`CenterCrop`/`ToTensor`/`Normalize`）在**主进程单线程**跑。`ps -o etime,time` 显示 **CPU 时间 = 墙上时间 × 14.6**（15 核全满），GPU 只是在等数据。**4 视图还凑合，8 视图 @288 要 ~1.9 小时 —— 等于用 GPU 的价格跑 CPU 的活。** 修法：把变换挪到 GPU（`torch.nn.functional.interpolate`），或限制视图数 |
| 7.9 | **给多视图 TTA 估时间时估错了一个数量级** | 我说"20 分钟"，实际 1.9 小时 | 别按 GPU 吞吐估。**先跑 `ps -o etime,time -p $(pgrep -f infer.py\|head -1)`**：`TIME ÷ ELAPSED ≈ 核数` ⇒ **CPU 瓶颈**；远小于核数 ⇒ GPU 瓶颈。两种情况的耗时差 5~10 倍 |
| 7.10 | **12 天的实例差点因为余额耗尽被掐断** | 控制台红字「余额不足24小时」 | 长赛程要**提前充值**。AutoDL 余额耗尽会关机，连续关机 15 天释放实例 |
...
- **天花板认知**：**64.16 是这套配方的天花板**；TTA 和 288 各是**结构性变量**，把它们抬到了 **69.218**。

---

### 16.11 ⭐⭐ 288 + TTA = **69.218**（2026-09-25 实测）

| 提交 | 配置 | 分数 | 累计 Δ |
| --- | --- | --- | --- |
| `sub_ep20.csv` | 224，无 TTA | 64.16 | — |
| `sub_tta4.csv` | 224 + 4 视图 TTA | 66.2456 | +2.09 |
| **`sub_288_tta4.csv`** | **288 + 4 视图 TTA** | **69.218** | **+5.06** |

#### 超加性：288 和 TTA 是互补的，不是简单叠加

```
预期（简单相加）：64.16 + 1.80（288 的 val_acc 领先）+ 2.09（TTA 实测）= 68.05
实际：                                                                   69.218
                                                                      ─────────
超出加和：                                                                +1.17
```

**这解释了为什么 288 的 `val_acc` 领先（+1.8）值得当真**（§16.6 的推断是对的），
也说明**多尺度/多裁剪的 TTA 在高分辨率下收益更大**（裁剪覆盖的像素更多，视图之间的差异性更有价值）。

#### 生成命令（用的就是本仓库最新代码）

```bash
python infer.py --test /root/autodl-tmp/test --checkpoint outputs_288/ep20.pt \
  --output sub_288_tta4.csv --tta          # 裸 --tta = DEFAULT_TTA = 那 4 个视图
```

日志里的证据（缺一不可）：

```
img_size=288: positional grid resampled to 9x9
TTA views (4) at 288px: plain, flip, wide, tight
wrote 37444 rows to sub_288_tta4.csv
```
...
| # | 做什么 | 依据 / 状态 |
| --- | --- | --- |
| **P0** | **`320 + 8 视图`** —— 先修 `infer.py` 的 CPU 瓶颈（见 §16.12） | TTA 是唯一还没测过「视图数」的杠杆。4 视图已值 +2.09~+2.97；8 视图 @320 在修好之后只要几分钟 |
| **P1** | **352 分辨率**（探针已验证，余弦 0.9581） | 288→320 还有 +0.75，说明还没饱和。但 352 的成本更高（每轮 ~9.5 分钟，20 轮 3.2 小时），且探针相似度最低 ⇒ **期望 +0.2~0.5** |
| **P1** | `320 + 4 视图` 的 **ep4 版本**（零成本备选） | ep4 与 ep20 已实测同分（§16.1） |
| **❌** | ~~`288 + 8 视图`（未修加速前）~~ | 试过，被杀掉了 —— CPU 变换要 ~1.9 小时（§16.7 坑 7.8） |
| **❌** | `sub_tta8.csv`（224 + 8 视图，已生成） | 天花板 ~67 < 已入账的 69.968 ⇒ **改善不了成绩** |
| **❌** | 再回头动 224、训练轮次、死类、取景框 | §16.1 / §16.2 / §16.3 |

---

### 16.12 ⭐⭐ 320 + TTA = **69.968**（2026-09-26 实测）

| 提交 | 配置 | 分数 | Δ |
| --- | --- | --- | --- |
| `sub_ep20.csv` | 224，无 TTA | 64.16 | — |
| `sub_tta4.csv` | 224 + 4 视图 | 66.2456 | +2.09 |
| `sub_288_tta4.csv` | 288 + 4 视图 | 69.218 | +2.97 |
| **`sub_320_tta4.csv`** | **320 + 4 视图** | **69.968** | **+0.75** |

**分辨率的收益递减但没停**（224→288 值 +2.97，288→320 值 +0.75）。

#### ⚠️ 又一次：`val_acc` 漏报了真实提升，`val_acc_hi` 跟上了

| | 288 | 320 | Δ |
| --- | --- | --- | --- |
| `val_acc` | 0.7256 | 0.7259 | **+0.0003**（几乎不动） |
| **`val_acc_hi`** | 0.8935 | **0.8988** | **+0.0053** |
| `val_loss` | 1.6535 | 1.6358 | −0.0177 |
| **平台分** | 69.218 | **69.968** | **+0.75** |

**`val_acc` 完全没动，真实分数涨了 0.75。** 这第三次印证了 §16.1 的结论 —— 但这个数据集上
**`val_acc_hi`（高置信子集的准确率）的变动方向和幅度都更接近真实变化**。

> ⚠️ **只有 288 vs 320 两个数据点，不足以当结论。** 但以后比较同族配置时，
> **优先看 `val_acc_hi` 和 `val_loss`，别只看 `val_acc`。**

#### 训练细节

- 每轮 **442~454 秒**（224 是 235 秒，2.0×；288 是 362 秒）
- 20 轮约 **2.5 小时**
- **显存 14.2 GB / 24 GB**
- 中途被「余额不足」强制关机，从第 16 轮 `--resume` 无损接上（§16.7 坑 7.10）

#### ⚠️ 下一步的唯一瓶颈：`infer.py` 的 CPU 变换

**TTA 是唯一还没测过「视图数」的杠杆。** 4 视图已经值 +2.09~+2.97，8 视图从没跑成过 ——
因为 `infer.py` 的图片变换在**主进程单线程**跑，8 视图 @288 要 **~1.9 小时**（§16.7 坑 7.8）。

**修法**（按优先级）：

1. **用 `DataLoader(num_workers=N)` 把变换并行化**（保留现有变换，语义完全不变，最安全）；
2. worker 里 `torch.set_num_threads(1)`，避免线程争抢（现在 `TIME/ELAPSED ≈ 14.6` 说明 15 核被
   低效的 torch 小算子多线程吃满了）；
3. 更彻底但会改变语义：**只 resize 一次，再从大图裁出各个视图**（现在是每个视图都从原图重新
   `Resize` 一次）。收益大，但 TTA 的数值会变，需重新验证。

修完后，**8 视图 @320 应该几分钟就能出结果**，那时才谈得上测「更多视图有没有用」。

---

### 16.13 ⭐⭐ 320 + 8 视图 = **70.332**，以及 `infer.py` 的 15 倍加速
...
| `sub_288_tta4.csv` | 288 + 4 视图 | 69.218 | +2.97 |
| `sub_320_tta4.csv` | 320 + 4 视图 | 69.968 | +0.75 |
| **`sub_320_tta8.csv`** | **320 + 8 视图** | **70.332** | **+0.364** |

**视图数确实是个杠杆（8 > 4），但和分辨率一样在递减。**

| 杠杆 | 第一步 | 第二步 |
| --- | --- | --- |
| 分辨率（224 → 288 → 320） | **+2.97** | **+0.75** |
| 视图数（4 → 8，在 320 上） | — | **+0.364** |

#### `infer.py` 的并行变换：**15 倍加速**

§16.12 末尾说「修完后 8 视图应该几分钟」—— **修完确实是这样**：

| | CPU 用量 | 8 视图 @320 耗时 |
| --- | --- | --- |
| 旧代码（单进程） | `TIME/ELAPSED ≈ 14.6` 核，一个进程硬扛 | 预估 **2.3 小时** |
| **新代码（8 worker）** | `56.4 CPU-分 ÷ 9.15 分 = 6.2` 核 | **9 分 9 秒** |

**改动内容**（变换语义一行未改）：

1. 新增 `TTADataset` —— 一次解码，V 个视图堆成一个张量交给 worker；
2. 新增 `_one_thread_per_worker()` —— worker 里 `torch.set_num_threads(1)`，8 个单线程进程
   比"一个进程开 15 个线程做小算子"快得多；
3. 推理循环改用 `DataLoader(num_workers=N)`；
4. 新增 `--workers`（默认 8）；
5. `prefetch_factor=1` —— 多视图的 batch 是 `(B, V, 3, S, S)`，比单视图大 V 倍，不能双倍预取；
6. `selftest.py` 的 4 处 infer 调用传 `--workers 0`。

**⚠️ 两条回归护栏（都要守住）：**

- `selftest.py` 里那句 **`--tta plain` 必须逐字节等于旧变换**的断言，改完仍然通过；
- **真机 md5 回归**：用新代码在 `outputs_320/ep20.pt` 上跑 `--tta`（4 视图），
  输出与已提交的 `sub_320_tta4.csv` **md5 完全相同**（`f5c99066da4d013a5295478d7e1ac4b8`）。
  ⇒ 并行化**逐字节零副作用**。

> ⚠️ **`--workers` 与 `--batch-size` 会一起决定内存**：worker 的预取 buffer 是
> `batch_size × V × 3 × S × S × 4` 字节。默认（256 / 8 视图 / 320px）约 **2.5 GB / batch**，
> 8 个 worker 最多同时在飞 8 个 —— 在 80 GB 内存的机器上没问题，小内存机器要调小 `--batch-size`。

#### 下一步

| # | 做什么 | 依据 |
| --- | --- | --- |
| **P0** | **352 分辨率**（探针已验证，余弦 0.9581） | 分辨率是至今**最强**的杠杆（+2.97、+0.75），且 288→320 还在涨。每轮约 9.5 分钟，20 轮约 **3.2 小时**。配 8 视图 |
| **P1** | 更多视图 / 更多尺度（12、16 视图） | 递减（+0.364/4视图），但便宜（15 分钟/次）⇒ **可能再挤 +0.1~0.2** |
| **P1** | `320 + 8 视图` 的 **ep4 版本** | ep4 与 ep20 已实测同分（§16.1），零成本备选 |
| **❌** | 训练轮次、死类、取景框、EMA 教师、224 那一档 | §16.1 / §16.2 / §16.3 / §16.4 |
