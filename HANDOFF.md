...
| **🥇 当前最好成绩** | **69.968** —— `sub_320_tta4.csv` = **320 分辨率 + 4 视图 TTA**（见 §16.12） |
| 🥈 | 69.218 —— `sub_288_tta4.csv` = 288 + 4 视图 TTA |
| 🥉 | 66.2456 —— `sub_tta4.csv` = 224 + 4 视图 TTA |
| 基线 | 64.16 —— `sub_ep20.csv` = 224，无 TTA |
| 初赛得分（参考） | 68.02 —— **不同数据集，不可直接比较**（**但现在已经超过了**） |
| 已完成 | `outputs_r2` / `outputs_old`（各 20 ep）、`outputs_ema`（224 + 教师）、`outputs_288`、死类诊断、8 视图 TTA 代码 |
| **下一步** | ① 抢 GPU → 生成 **`288 + 8 视图`** → 提交（最大期望值）② 抢不到卡就交已备好的 `sub_tta8.csv` ③ 再往后试 **320 分辨率**（探针已验证可行） |
