...
> 交接文档。最后更新：**2026-09-23**。
> 面向：接手继续推进的工程师。假设你会 PyTorch、用过 Linux 命令行，但不了解这个赛题和之前发生过什么。
>
> 最新状态看 **§0** 和 **§15**（复赛实测结果）。§6~§13 是初赛阶段的记录，仍然有效但要结合 §15 读。
...
| 项目 | 状态 |
| --- | --- |
| 当前阶段 | **复赛（750 类）**，提交截止 2026-10-07 前后（约两周） |
...
| 已完成 | 复赛**两条线各 20 epoch 全部训完**（新默认 `outputs_r2` / 旧行为 `outputs_old`） |
| 平台实际得分 | **64.16**（复赛第一次提交，交的是 `sub_ep20.csv`，见 §15.6） |
| 初赛得分（参考） | 68.02 —— **不同数据集，不可直接比较** |
| 基线自测指标 | `val_acc = 0.7070`（**这个数高估 6.54 点**，见 §7 和 §15.4） |
| 复赛的算法改动 | 抗记忆三件套已跑完，**结果有了但尚未定论**（见 §15.1 / §15.2） |
| 下一步 | 提交 `sub_ep4.csv` 分开 H1/H3 → 诊断"卡死的类" → 调低 `--tau-conf` |

**最重要的一句话**：复赛两条线都训完了、有了第一个真实分数 **64.16**。本会话最有价值的产出是**证伪了一个假设**（H2，§15.4）——原以为缺口是度量口径造成的、可以用本地指标代替排行榜，**实测只值 0.7/6.54 点，这条路作废**。剩下的 5.84 点仍未归因，**每天 2 次提交名额是现在最紧的约束**（比 GPU 时间紧得多）。

**第二个值得注意的点**（§15.5）：有一批类从第 4 轮到第 20 轮**召回率恒为 0.00**，而且是**大类**（val 里 `n=24`，即训练集最大档）。这不像训练不足，可能是目前最有希望的方向。
